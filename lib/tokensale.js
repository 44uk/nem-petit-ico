...
const MARKETS = require('./markets.js').markets;
...
function filterIncomingTransfer(txes) {
  ...
  .filter(tx => {
    let sender = helper.pubkey2addr(tx.signer);
    if (MARKETS.includes(sender)) {
      logger.warn(`MarketAddress: ${sender}, Type: ${tx._type}, Hash: ${tx._hash}`);
      return false;
    }
    return (
      tx.type === 257 && tx.recipient === SINK && sender !== SINK &&
...
function sumXemAmount(txes) {
  ...
  let applicants = [];
  logger.debug('---- each applicant sums ----');
...
function sumMosaicQuantity(txes) {
  let groups = _.groupBy(txes, 'recipient');
  let recipients = [];
  logger.debug('---- each sent mosaic sums ----');
  Object.keys(groups).forEach(key => {
    let sum = groups[key].reduce((n, t) => {
      if((t.version & 0x00ffffff) === 2) {
    ...
    logger.debug(`${sum}(${MOSAIC_FQN}) => ${key}`);
  });
  return recipients;
}
...
function pApplicantAddresses() {
  return helper.fetchTx('incoming')(SINK, MIN_TIME)
...
function pSentMosaicAddresses() {
  return helper.fetchTx('outgoing')(SENDER, MIN_TIME)
    .then(filterOutgoingTransfer)
    .then(sumMosaicQuantity)
  ;
}
function pSendingMosaicAddresses() {
  return helper.fetchTx('unconfirmed')(SENDER, MIN_TIME)
    .then(filterOutgoingTransfer)
    .then(sumMosaicQuantity)
  ;
}
...
function main() {
  ...
  Promise.all([
    ...
    logger.info('---- Sents ----');
    logger.info(sents);
    logger.info('---- Sendings ----');
    logger.info(sendings);

    sents = mergeSent(sents, sendings)
    logger.info('---- Merged Sents ----');
    logger.info(sents);

    let candidates = selectCandidates(applicants, sents);
    ...
    if(DRYRUN == 0) {
      return Promise.all(candidates.map(c => {
        logger.info(`${c.quantity}(${MOSAIC_FQN}) => ${c.address}`);
