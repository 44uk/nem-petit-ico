...
function sendMosaic(address, fqn, pkey) {
  let splitedFqn = splitNsAndMo(fqn);
  let ns = splitedFqn[0];
  let mo = splitedFqn[1];
  let common = nem.model.objects.create('common')('', pkey);
  let transferTx = nem.model.objects.create('transferTransaction')(address, 1, '');
  let moDefMetaDataPair = nem.model.objects.get('mosaicDefinitionMetaDataPair');
  // let moAttachment1 = nem.model.objects.create('moAttachment')('nem', 'xem', 1);
  // transferTx.mosaics.push(moAttachment1);
  let moAttachment2 = nem.model.objects.create('mosaicAttachment')(ns, mo, 1);
  transferTx.mosaics.push(moAttachment2);

  return nem.com.requests.namespace.mosaicDefinitions(ENDPOINT, moAttachment2.mosaicId.namespaceId).then((res) => {
    let neededDefinition = nem.utils.helpers.searchMosaicDefinitionArray(res.data, [mo]);
    fullMosaicName = nem.utils.format.mosaicIdToName(moAttachment2.mosaicId);
    // if(undefined === neededDefinition[fullMosaicName]) return logger.error(`${mo} Mosaic not found!`);
    ...
    // return new Promise((resolve) => { resolve(txEntity); });
    // return Promise.resolve(txEntity);
