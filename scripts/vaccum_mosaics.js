...
function sendMoz(key) {
  let addr = privkey2addr(key);
  return nem.com.requests.account.mosaics.owned(endpoints.pick(), addr)
    .then(res => {
      let moas = res.data.find(moa => {
        return (
          moa.mosaicId.namespaceId === 'y4uk' &&
          moa.mosaicId.name === 'gift'
        );
      });
      if(moas) {
        return moas;
      } else {
        throw new Error('No mosaic');
      }
    })
    .then(moa => {
      return nem.com.requests.namespace.mosaicDefinitions(endpoints.pick(), moa.mosaicId.namespaceId)
        .then(res => {
          let neededDefinition = nem.utils.helpers.searchMosaicDefinitionArray(res.data, ['gift']);
          moa.def = neededDefinition
          return moa;
        })
    })
    .then(mo => {
      ...
      let mosaicAttachment = nem.model.objects.create('mosaicAttachment')(
        mo.mosaicId.namespaceId,
        mo.mosaicId.name,
        mo.quantity
      );
      transferTx.mosaics.push(mosaicAttachment);

      let fullMosaicName = nem.utils.format.mosaicIdToName(mo.mosaicId);
      let moDefMetaDataPair = nem.model.objects.get('mosaicDefinitionMetaDataPair');
      moDefMetaDataPair[fullMosaicName] = {};
      moDefMetaDataPair[fullMosaicName].mosaicDefinition = mo.def[fullMosaicName];
