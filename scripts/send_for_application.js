...
function send(keys) {
  keys.forEach(key => {
    let asMosaic = randNum(1, 2) === 1 ? true : false;
    sendXem(key, 1, asMosaic)
      .then(data => { console.log(data) })
      .catch(err => { console.error(err) })
  });
}

function sendXem(key, amount, asMosaic) {
  ...
  if(asMosaic) {
    let moAttachment = nem.model.objects.create('mosaicAttachment')(
      'nem',
      'xem',
      amount * 1000000
    );
