...
function send(keys) {
  keys.forEach(key => {
    switch(randNum(1, 2)) {
      case 1:
        // sendXem(key, randNum(1, 3))
        sendXem(key, 0.1)
          .then(data => { console.log(data) })
        ;
        break;
      case 2:
        // sendNemXem(key, randNum(1, 3))
        sendNemXem(key, 0.1)
          .then(data => { console.log(data) })
        ;
        break;
...
function sendXemMsig(key, amount) {
  let common = nem.model.objects.create('common')('', key);
  let transferTx = nem.model.objects.create('transferTransaction')(
    SINK,
    amount,
  ...
  transferTx.multisigAccount = {publicKey: '1a800b52532a96634e66ea1f6d276bba20d465a4e6cc98b861c3099db28a5f29'};

  let txEntity = nem.model.transaction.prepare('transferTransaction')(
    common,
    transferTx,
    nem.model.network.data.testnet.id
  );
...
function sendNemXem(key, amount) {
  let common = nem.model.objects.create('common')('', key);
  let transferTx = nem.model.objects.create('transferTransaction')(
    SINK,
    1,
